...
export const CheckoutView: React.FC<CheckoutViewProps> = ({
  ...
  const handleProceedToCashfree = async (e?: React.FormEvent) => {
    ...
    try {
      ...
      // This opens Cashfree's hosted payment page (Card, UPI, NetBanking, Wallets)
      cashfree.checkout({
  ...
  const handleVerifyDirectPayment = async () => {
    if (!inputUtr || inputUtr.length < 12) {
      setPaymentError('Please enter a valid 12-digit UTR number.');
      return;
    }
    setIsCheckingPayment(true);
    setPaymentError('');
    // UTR based manual confirm
    setTimeout(() => {
      setIsCheckingPayment(false);
      setShowCashfreeModal(false);
      setSuccessOrderId(`ord_utr_${Date.now()}`);
      setConfirmedUtr(inputUtr);
      setPaymentStatus('success');
